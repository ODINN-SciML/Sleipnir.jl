...
mutable struct Climate2D{CLIMRAW <: RasterStack, CLIMRAWSTEP <: RasterStack, CLIMSTEP <: ClimateStep, CLIM2DSTEP <: Climate2Dstep, F <: AbstractFloat}
    ...
    function Climate2D(
        ...
        dummy_period = partial_year(Day, params.simulation.tspan[1]):Day(1):partial_year(Day, params.simulation.tspan[1] + params.simulation.step_MB)
...
function Base.show(io::IO, climate::Climate2D)
    ...
    print(io, " time steps [ ")
    printstyled(io, Dates.format(dims(climate.climate_raw_step, Ti)[begin], "yyyy-mm-dd");color=:green)
    print(io, " → ")
    printstyled(io, Dates.format(dims(climate.climate_raw_step, Ti)[end], "yyyy-mm-dd");color=:green)
    println(io, " ]")
    ...
    print(io, " time steps [ ")
    printstyled(io, Dates.format(dims(climate.raw_climate, Ti)[begin], "yyyy-mm-dd");color=:green)
    print(io, " → ")
    printstyled(io, Dates.format(dims(climate.raw_climate, Ti)[end], "yyyy-mm-dd");color=:green)
    print(io, " ]")
end

Base.show(io::IO, type::MIME"text/plain", climate_step::Climate2Dstep) = Base.show(io, climate_step)
function Base.show(io::IO, climate_step::Climate2Dstep)
    ...
    print(io, " in [ ")
    printstyled(io, "$(round(minimum(climate_step.x);digits=6))° → $(round(maximum(climate_step.x);digits=6))°"; color=:blue)
    println(io, " ]")

    print(io, "  y: ")
    printstyled(io, typeof(climate_step.y);color=:yellow)
    print(io, " in [ ")
    printstyled(io, "$(round(minimum(climate_step.y);digits=6))° → $(round(maximum(climate_step.y);digits=6))°"; color=:blue)
    println(io, " ]")

    println(io, "  min,mean,max:")

    print(io, "    PDD: ")
    printstyled(io, "$(round(minimum(climate_step.PDD); digits=1)) $(round(mean(climate_step.PDD); digits=1)) $(round(maximum(climate_step.PDD); digits=1))\n"; color=:blue)

    print(io, "    rain: ")
    printstyled(io, "$(round(minimum(climate_step.rain); digits=1)) $(round(mean(climate_step.rain); digits=1)) $(round(maximum(climate_step.rain); digits=1))\n"; color=:blue)

    print(io, "    snow: ")
    printstyled(io, "$(round(minimum(climate_step.snow); digits=1)) $(round(mean(climate_step.snow); digits=1)) $(round(maximum(climate_step.snow); digits=1))\n"; color=:blue)

    print(io, "    temp: ")
    printstyled(io, "$(round(minimum(climate_step.temp); digits=1)) $(round(mean(climate_step.temp); digits=1)) $(round(maximum(climate_step.temp); digits=1))\n"; color=:blue)
end
