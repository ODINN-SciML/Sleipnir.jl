...
function generate_inputs_testset()
    @testset "generate_inputs" begin
        (; A, B, C, incompleteInp) = MockTestInputs

        @test generate_inputs((a = A(), b = B(), c = C()), nothing, nothing, 3.0) ==
              (a = ones(3), b = range(1, 3), c = 3.0)
        @test generate_inputs((x = A(), y = B(), z = C()), nothing, nothing, 2.0) ==
              (x = ones(3), y = range(1, 3), z = 2.0)
        ...
        # type stability
        JET.@test_opt generate_inputs((a = A(), b = B(), c = C()), nothing, nothing, 3.0)
        JET.@test_opt generate_inputs((x = A(), y = B(), z = C()), nothing, nothing, 2.0)
    end
end

function normalize_law_inputs_testset()
    @testset "_normalize_law_inputs" begin
        (; A, B, C) = MockTestInputs

        @test _normalize_law_inputs((A(), B(), C())) == (a = A(), b = B(), c = C())
        @test _normalize_law_inputs((a = A(), b = B(), c = C())) ==
              (a = A(), b = B(), c = C())
        @test _normalize_law_inputs((x = A(), y = B(), z = C())) ==
              (x = A(), y = B(), z = C())
    end
end
...
function apply_vjp_law_testset()
    @testset "Law VJPs" begin
        (; A, B, C) = MockTestInputs
